...
static __device__ WCUComplex GPU_ampEtaPi0(GDouble par[7], int hel[3], GDouble inv[6], GDouble mass2[4], GDouble b_par,GDouble c0, GDouble c1, GDouble c2, GDouble n0, GDouble n1, GDouble n2, GDouble a0, GDouble a1, GDouble a2,GDouble a3,GDouble a4, GDouble S0, int fastParticle, int charge ){

	WCUComplex zero =  {0,0};

        if(abs(hel[0]*hel[1]*hel[2]) != 1 || hel[0]==0){return zero;}

	GDouble s, s12,phi,s23,t1,u3;
	GDouble m12, m22, m32, ma2;
phi = inv[0];	s   = inv[1];   s12 = inv[2];   s23 = inv[3];   t1  = inv[4];   u3  = inv[5];
...
if(charge ==0){
	...
	if(fastParticle==2){
	ADR1 = GPU_DoubleRegge(tau, s, si, alp,S0); // fast eta
	...
	if(fastParticle==3){
	ADR2 = GPU_DoubleRegge(tau, s, si, alp,S0); // fast pi0
...
GDouble term=a0 + a1*cos(phi) +a2*cos(2*phi) +a3*cos(3*phi) + a4*cos(4*phi);
        finalFactor*= term;
...
static __device__ WCUComplex GPU_calcAmplitude(GDouble phi, GDouble s, GDouble s12, GDouble s23, GDouble t1, GDouble u3, GDouble S0, GDouble b_par, GDouble c0, GDouble c1, GDouble c2, GDouble n0, GDouble n1, GDouble n2, GDouble a0, GDouble a1, GDouble a2, GDouble a3, GDouble a4,GDouble beamM2, GDouble p1M2, GDouble p2M2, GDouble recoilM2, int fastParticle, int charge){

	GDouble param[12] = {b_par, c0,c1,c2,n0,n1,n2,a0,a1,a2,a3,a4};
	GDouble inv[6] = {phi,s, s12, s23, t1 ,u3};
	GDouble mass2[4] = {beamM2, p1M2, p2M2, recoilM2};

	int hel[3] = {1,-1,-1};

	WCUComplex amp = GPU_ampEtaPi0(param, hel, inv, mass2, b_par, c0, c1,c2,n0,n1,n2,a0,a1,a2,a3,a4, S0, fastParticle, charge);
